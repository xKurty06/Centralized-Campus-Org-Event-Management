...
function KpiCard({ card }: { card: KpiCard }) {
    const inner = (
        <div className={`card card-body !p-5 flex items-start gap-4 ${card.href ? 'hover:shadow-md transition-shadow cursor-pointer' : ''}`}>
            <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${card.bg}`}>
                <span className={card.color}>{card.icon}</span>
            </div>
            <div className="flex-1 min-w-0">
                <p className="text-[12px] font-medium text-[var(--color-text-muted)] mb-0.5">{card.label}</p>
                <p className="text-[26px] font-bold text-[var(--color-text)] leading-none">{card.value}</p>
                {card.delta && (
                    <p className={`text-[11px] font-medium mt-1 ${card.positive ? 'text-green-600' : 'text-red-500'}`}>
    ...
    return card.href ? <Link href={card.href} className="no-underline">{inner}</Link> : inner;
}

/* — Event Status Mini Bar — */
...
function ActivityIcon({ type }: { type: ActivityItem['type'] }) {
    const map: Record<ActivityItem['type'], { bg: string; icon: React.ReactNode }> = {
        accreditation: { bg: 'bg-amber-100', icon: <svg className="w-3 h-3 text-amber-600" viewBox="0 0 24 24" fill="none"><path d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg> },
        event_created: { bg: 'bg-green-100', icon: <svg className="w-3 h-3 text-green-600" viewBox="0 0 24 24" fill="none"><path d="M12 4v16m8-8H4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" /></svg> },
        event_cancelled: { bg: 'bg-red-100', icon: <svg className="w-3 h-3 text-red-600" viewBox="0 0 24 24" fill="none"><path d="M6 18L18 6M6 6l12 12" stroke="currentColor" strokeWidth="2" strokeLinecap="round" /></svg> },
        user_deactivated: { bg: 'bg-gray-100', icon: <svg className="w-3 h-3 text-gray-500" viewBox="0 0 24 24" fill="none"><path d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg> },
    };
    const { bg, icon } = map[type];
    return <div className={`w-6 h-6 rounded-full ${bg} flex items-center justify-center flex-shrink-0`}>{icon}</div>;
}
...
export default function AdminDashboardPage() {
    ...
    const suspendedOrgs = PLACEHOLDER_ORGS.filter((o) => o.accreditationStatus === 'Suspended');
    ...
    return (
        <AdminShell>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                ...
                {/* Period selector */}
                <div className="flex items-center gap-1 p-1 rounded-lg bg-[var(--color-surface-2)] border border-[var(--color-border)] self-start sm:self-auto">
                    {(['week', 'month', 'all'] as const).map((p) => (
                        <button
                            key={p}
                            onClick={() => setPeriod(p)}
                            className={`text-[12px] font-medium px-3 py-1.5 rounded-md transition-all capitalize
                ${_period === p
            ...
            {/* ── Suspended Alert (if any) ── */}
            {suspendedOrgs.length > 0 && (
                <div className="mb-8 rounded-xl border border-amber-200 bg-amber-50 px-5 py-4 flex items-start gap-3">
                    <svg className="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none">
                    ...
                    <div className="flex-1">
                        <p className="text-[13px] font-semibold text-amber-800">
                            {suspendedOrgs.length} organization{suspendedOrgs.length > 1 ? 's are' : ' is'} currently suspended
                        </p>
                        <p className="text-[12px] text-amber-700 mt-0.5">
                            {suspendedOrgs.map((o) => o.name).join(', ')} — suspended organizations cannot publish events.
                        </p>
                    </div>
                    <Link
                        href="/admin/organizations"
                        className="text-[12px] font-semibold text-amber-700 hover:text-amber-900 no-underline flex-shrink-0 mt-0.5"
                    >
                        Review →
                    </Link>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6">
                ...
                <div className="card overflow-hidden">
                    <div className="px-5 py-4 border-b border-[var(--color-border)] flex items-center justify-between">
                        <h2 className="text-[14px] font-semibold text-[var(--color-text)]">Recent Events</h2>
                        <Link href="/admin/events" className="text-[12px] font-medium text-[var(--color-primary)] no-underline hover:underline">
                            View all
                        </Link>
                    </div>
                        ...
                        <table className="table-base">
                            ...
                            <tbody>
                                {PLACEHOLDER_EVENTS.map((ev) => (
                                    <tr key={ev.id}>
                                        <td>
                                            <p className="text-[13px] font-semibold text-[var(--color-text)] truncate max-w-[180px]">{ev.title}</p>
                                        </td>
                                        <td className="hidden sm:table-cell">
                                            <p className="text-[12px] text-[var(--color-text-muted)] truncate max-w-[140px]">{ev.orgName}</p>
                                        </td>
                                        <td className="hidden md:table-cell">
                                            <p className="text-[12px] text-[var(--color-text-muted)]">{formatDate(ev.startDate)}</p>
                                        </td>
                                        <td>
                                            <div className="w-[100px]">
                                                <StatusMiniBar registered={ev.registered} capacity={ev.capacity} />
                                            </div>
                                        </td>
                                        <td>
                ...
                <div className="card overflow-hidden">
                    <div className="px-5 py-4 border-b border-[var(--color-border)]">
                        <h2 className="text-[14px] font-semibold text-[var(--color-text)]">Recent Admin Activity</h2>
                    </div>
                    <div className="divide-y divide-[var(--color-border)]">
                        {PLACEHOLDER_ACTIVITY.map((item) => (
                            <div key={item.id} className="flex items-start gap-3 px-5 py-3.5">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-[12px] text-[var(--color-text)] leading-snug">{item.target}</p>
                                    <p className="text-[11px] text-[var(--color-text-muted)] mt-0.5">{item.actor} · {item.timestamp}</p>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="mt-6 card overflow-hidden">
                <div className="px-5 py-4 border-b border-[var(--color-border)] flex items-center justify-between">
                    <h2 className="text-[14px] font-semibold text-[var(--color-text)]">Organizations Overview</h2>
                    <Link href="/admin/organizations" className="text-[12px] font-medium text-[var(--color-primary)] no-underline hover:underline">
                        Manage all →
                    </Link>
                </div>
                <div className="overflow-x-auto">
                                ...
                                <tr key={org.id}>
                                    <td>
                                        <div className="flex items-center gap-2.5">
                                            ...
                                            <p className="text-[13px] font-semibold text-[var(--color-text)] truncate max-w-[160px]">{org.name}</p>
                                        </div>
                                    </td>
                                    <td><span className={`badge ${CATEGORY_STYLES[org.category]}`}>{org.category}</span></td>
                                    <td className="hidden sm:table-cell">
                                        <span className="text-[13px] text-[var(--color-text-muted)]">{org.eventCount}</span>
                                    </td>
                                    <td className="hidden md:table-cell">
                                        <span className="text-[12px] text-[var(--color-text-muted)]">{formatDate(org.accreditedAt)}</span>
                                    </td>
                                    <td>
