...
const CATEGORY_COLORS: Record<EventCategory, string> = {
  Workshop: 'bg-blue-50 text-blue-700', Seminar: 'bg-purple-50 text-purple-700', Competition: 'bg-orange-50 text-orange-700',
  Training: 'bg-yellow-50 text-yellow-800', Outreach: 'bg-teal-50 text-teal-700', Cultural: 'bg-pink-50 text-pink-700',
  Activity: 'bg-indigo-50 text-indigo-700', Other: 'bg-gray-100 text-gray-600',
};
  ...
  useEffect(() => {
    (async () => {
      const [orgRes, eventsRes] = await Promise.all([
        fetch(`${API_BASE_URL}/organizations/${id}`, { headers: { Accept: 'application/json' } }).catch(() => null),
        fetch(`${API_BASE_URL}/events?org_id=${encodeURIComponent(id)}&per_page=100`, { headers: { Accept: 'application/json' } }).catch(() => null),
      ]);
      const orgPayload = await orgRes?.json().catch(() => null) as { success?: boolean; data?: any } | null;
      const evPayload = await eventsRes?.json().catch(() => null) as { success?: boolean; data?: any[] } | null;
      ...
      const banner = ['bg-blue-100', 'bg-purple-100', 'bg-orange-100', 'bg-teal-100', 'bg-green-100', 'bg-pink-100'];
      ...
      setOrg({
        ...
        established: String(orgPayload.data.founded_date ?? '').slice(0, 4) || 'N/A',
        status: orgPayload.data.accreditation_status === 'Suspended' ? 'Suspended' : 'Active',
        color: colorMap[orgPayload.data.category_name ?? 'Non-Academic'] ?? 'bg-gray-100 text-gray-700',
        officers: [],
        events: mappedEvents,
      });
      setLoading(false);
      ...
      <main className="flex-1 w-full max-w-[1280px] mx-auto px-6 lg:px-12 py-8 flex flex-col gap-6">
        ...
        <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-xl p-1 w-fit">
          {(['about', 'officers', 'events'] as const).map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`px-5 py-2 rounded-lg text-[13px] font-medium transition-all duration-150 cursor-pointer whitespace-nowrap ${activeTab === tab ? 'bg-green-700 text-white shadow-sm' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-100'}`}>{tab[0].toUpperCase() + tab.slice(1)}</button>
          ))}
        </div>

        {activeTab === 'about' && <div className="bg-white rounded-xl border border-gray-200 p-5"><p className="text-[14px] text-gray-600 leading-relaxed whitespace-pre-line">{org.description}</p></div>}
            ...
            {org.events.map((event) => (
              <Link key={event.id} href={`/events/${event.id}`} className="group bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 no-underline flex flex-col">
                ...
                <div className="p-4 flex flex-col gap-2 flex-1">
                  <span className={`self-start text-[10px] font-semibold px-2 py-0.5 rounded-md ${CATEGORY_COLORS[event.category]}`}>{event.category}</span>
                  <h3 className="text-[13px] font-semibold text-gray-900 group-hover:text-green-700 transition-colors line-clamp-2 leading-snug">{event.title}</h3>
                  <div className="flex items-center gap-1.5 text-[12px] text-gray-400 mt-auto">{formatDate(event.date)} · {event.venue}</div>
                </div>
              </Link>
            ))}
          </div>
