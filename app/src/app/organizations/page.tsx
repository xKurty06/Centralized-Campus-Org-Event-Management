...
> = {
  All: {
    description: "All accredited campus organizations",
    color: "text-gray-700",
    bg: "bg-gray-100 border-gray-300",
  },
  Academic: {
    description: "Department-based and discipline-specific organizations",
    color: "text-blue-700",
    bg: "bg-blue-50 border-blue-200",
  },
  "Non-Academic": {
    description:
      "Student government, civic, cultural, and special interest groups",
    color: "text-orange-700",
    bg: "bg-orange-50 border-orange-200",
  },
  Religious: {
    description: "Faith-based communities and campus ministry organizations",
          ...
          {options.map((opt) => {
            ...
            return (
              <button
                ...
                className={`flex items-center justify-between w-full px-4 py-2.5 text-[13px] font-medium text-left transition-colors cursor-pointer ${opt.value === value ? "bg-green-50 text-green-700" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`}
              >
                <span>{opt.label}</span>
                <span className="flex items-center gap-2">
                  {typeof count === "number" && (
                    <span
                      className={`text-[11px] font-semibold px-1.5 py-0.5 rounded-full ${opt.value === value ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                    >
                      {count}
                    </span>
                  )}
  ...
  return (
    ...
    >
      <div className="px-5 pt-5 pb-4 flex items-start gap-4">
        <div
          className={`w-12 h-12 rounded-md flex items-center justify-center flex-shrink-0 text-[13px] font-bold overflow-hidden ${org.color}`}
        >
          {org.logoUrl ? (
            <img
              src={org.logoUrl}
      ...
      <div className="mt-auto px-5 py-3 border-t border-gray-100 flex items-center justify-between">
        ...
        <svg
          className="w-4 h-4 text-gray-300 group-hover:text-green-600 group-hover:translate-x-0.5 transition-all"
          viewBox="0 0 20 20"
          fill="none"
        >
          <path
...
function CategoryHeader({ category }: { category: OrgCategory }) {
  ...
  return (
    <div
      className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${meta.bg} mb-4`}
    >
      <p className={`text-[14px] font-bold ${meta.color}`}>
        {category} Organizations
      </p>
      <span className="text-gray-300">·</span>
      <p className={`text-[12px] ${meta.color} opacity-70`}>
        {meta.description}
      </p>
    </div>
  );
...
export default function OrganizationsPage() {
  ...
  async function loadOrganizations(showLoading = true) {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/organizations?per_page=500`, {
      ...
      setOrgs(
        payload.data.map((o) => ({
          ...
          status: (o.accreditation_status === "Suspended"
            ? "Suspended"
            : "Active") as OrgStatus,
          color:
            colorMap[o.category_name ?? "Non-Academic"] ??
  ...
  const baseFiltered = useMemo(
    () =>
      searchedOrgs.filter((o) => !statusFilter || o.status === statusFilter),
  ...
  const filtered = useMemo(
    () =>
      activeTab === "All"
        ? baseFiltered
        : baseFiltered.filter((o) => o.category === activeTab),
  ...
  const categoryCounts = useMemo(
    () => ({
      ...
      "Non-Academic": baseFiltered.filter((o) => o.category === "Non-Academic")
        .length,
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-[14px] text-gray-500 mt-1">
              Discover and join accredited CvSU student organizations.
            </p>
          </div>
          <div className="flex items-center gap-4 flex-shrink-0">
            ...
            <div className="flex flex-col items-end">
              <span className="text-[22px] font-bold text-gray-700 leading-none">
                {orgs.length}
              </span>
        ...
        <div className="bg-white rounded-xl border border-gray-200 px-4 py-3 flex flex-col gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {hasFilters && (
              <button
                onClick={clearAll}
                className="text-[12px] font-medium text-red-500 hover:text-red-600 px-2.5 py-1.5 rounded-lg hover:bg-red-50 transition-colors cursor-pointer"
              >
                Clear all
              </button>
            )}
          ...
          {CATEGORY_TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-3.5 py-2 rounded-lg border text-[13px] font-medium transition-all duration-150 cursor-pointer ${active ? "bg-green-700 text-white border-green-700" : "bg-white text-gray-600 border-gray-200 hover:border-green-600 hover:text-green-700"}`}
              >
                <span>{tab.label}</span>
                <span
                  className={`inline-flex h-5 min-w-5 items-center justify-center rounded-full px-1.5 text-[11px] font-semibold ${active ? "bg-white/20 text-white" : "bg-gray-100 text-gray-500"}`}
                >
