...
function SuccessContent() {
  ...
  // 2. Exact validation using flexible regex (matches 'onsite', 'on-site', 'Onsite', etc.)
  const isOnsiteChosen = /onsite|on-site/i.test(rawMethod);

  // 3. Confirm payment rules based directly on event tiering
  const isPaidEvent = event.type === 'Paid' || event.fee > 0;
  
  /* 4. Determine state variant reliably */
