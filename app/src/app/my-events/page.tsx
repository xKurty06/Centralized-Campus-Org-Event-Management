...
function getStatusPill(reg: MyRegistration) {
  ...
  if (
    ...
    reg.proof_status === "Pending_Review"
  )
    return {
      label: "Under review",
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <div className="flex flex-col sm:flex-row">
        <div className="bg-gray-100 sm:w-[100px] h-24 sm:h-auto flex-shrink-0 flex items-center justify-center relative">
          {countdown && (
            <span className="absolute top-2 left-2 text-[10px] font-bold bg-green-700 text-white px-2 py-0.5 rounded-full">
              {countdown}
            </span>
          )}
        </div>
        <div className="flex-1 p-4 flex flex-col gap-2 min-w-0">
          <div className="flex flex-wrap items-center gap-1.5">
            <span
              className={`text-[11px] font-semibold px-2 py-0.5 rounded-md ${CATEGORY_COLORS[reg.category_name] ?? CATEGORY_COLORS.Other}`}
            >
              {reg.category_name}
            </span>
            <span
              className={`flex items-center gap-1 text-[11px] font-semibold px-2.5 py-0.5 rounded-full border ${statusPill.style}`}
            >
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-[12px] text-gray-500">
            <span>
              ...
              {formatTime(reg.event_start_date)} -{" "}
              {formatTime(reg.event_end_date)}
            </span>
...
export default function MyEventsPage() {
    ...
    (async () => {
      ...
      setRows(
        data.map((r: any) => ({
          ...
          event_start_date: String(
            r.event_start_date ?? new Date().toISOString(),
          ),
          event_end_date: String(
            r.event_end_date ?? r.event_start_date ?? new Date().toISOString(),
          ),
          ...
          proof_status: (r.proof_status ?? undefined) as
            | ProofStatus
            | undefined,
        })),
      );
  ...
  const totalConfirmed = rows.filter(
    (r) => !r.event_is_paid || r.payment_status === "Paid",
  ).length;
  const totalPending = rows.filter(
    (r) => r.event_is_paid && r.payment_status === "Pending",
  ).length;
  const totalNeedAction = rows.filter(needsUpload).length;
  const totalCheckedIn = rows.filter(
          ...
          <>
            ...
            {totalNeedAction > 0 && (
              <div className="text-[12px] text-amber-800 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
                {totalNeedAction} registration
                {totalNeedAction > 1 ? "s require" : " requires"} proof of
                payment.
              </div>
            )}
            <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-xl p-1 w-fit">
              {(
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-5 py-2 rounded-lg text-[13px] font-medium ${activeTab === tab.key ? "bg-green-700 text-white" : "text-gray-500 hover:text-gray-800 hover:bg-gray-100"}`}
                >
                  {tab.label}
                  <span
                    className={`text-[11px] font-semibold px-1.5 py-0.5 rounded-full ${activeTab === tab.key ? "bg-white/20 text-white" : "bg-gray-100 text-gray-500"}`}
                  >
                    {tab.count}
                  </span>
                </button>
