...
type GlobalRole = "Super_Admin" | "Overseer" | "Officer" | "User";
...
function InfoRow({
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 py-3 border-b border-gray-50 last:border-b-0">
      <span className="text-[12px] font-medium text-gray-400 flex-shrink-0 sm:w-44">
        {label}
      </span>
      <span
        className={`text-[13px] font-semibold text-gray-800 sm:text-right ${mono ? "font-mono tracking-wide" : ""}`}
      >
        {value}
      </span>
    </div>
...
function StatCard({
  ...
  return (
    <div
      className={`rounded-xl border ${bg} px-4 py-3.5 flex flex-col gap-0.5`}
    >
      <span className={`text-[24px] font-bold leading-none ${color}`}>
        {value}
      </span>
...
export default function ProfilePage() {
    ...
    const run = async () => {
      try {
        ...
        setUser({
          ...
          department: {
            name: u.department?.name ?? "N/A",
            code: u.department?.code ?? "N/A",
          },
          course: u.course
            ? {
              id: String(u.course.id ?? ""),
              name: u.course.name ?? "N/A",
              code: u.course.code ?? "N/A",
            }
            : null,
        ...
        if (Array.isArray(u.memberships)) {
          setMemberships(
            u.memberships.map((m: any) => ({
              ...
              org_category:
                m.org_category === "Academic" ||
                  m.org_category === "Non-Academic" ||
                  m.org_category === "Religious"
                  ? m.org_category
                  : "Non-Academic",
              ...
              paid_membership_fee: Boolean(m.paid_membership_fee ?? false),
            })),
          );
        }
      } catch (error: any) {
  ...
  const isOfficer = useMemo(
    () =>
      user?.global_role === "Officer" ||
      memberships.some((m) => m.is_active && m.position !== "Member"),
    [user, memberships],
  );
  ...
  if (isLoading || !user)
    return (
      <div className="min-h-screen bg-gray-50">
        <main className="w-full max-w-[1280px] mx-auto px-6 lg:px-12 py-8">
          <ProfilePageSkeleton />
        </main>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <main className="flex-1 w-full max-w-[1280px] mx-auto px-6 lg:px-12 py-8 flex flex-col gap-6 animate-content-reveal">
        <div>
          <h1 className="text-[26px] font-bold text-gray-900 tracking-tight">
            My Profile
          </h1>
          <p className="text-[14px] text-gray-500 mt-1">
            Your verified campus identity. This information is pulled from
            institutional records and cannot be edited here.
          </p>
        </div>
          ...
          <div className="px-6 pb-6">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 -mt-11 mb-4">
              ...
              <div className="flex flex-wrap items-center gap-2 pb-1">
                <span
                  className={`flex items-center gap-1.5 text-[11px] font-semibold border px-3 py-1 rounded-full ${user.is_active ? "bg-green-50 text-green-700 border-green-200" : "bg-red-50 text-red-500 border-red-200"}`}
                >
            ...
            <p className="text-[13px] text-gray-400 mt-1">{departmentName}</p>
            <p className="text-[13px] text-gray-400 mt-0.5">
              {user.course?.name ?? "Course unavailable"} (
              {user.course?.code ?? "N/A"} {user.year_level ?? "-"}-
              {user.section ?? "-"})
            </p>
          </div>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <StatCard
            value={activity.total_confirmed}
            label="Confirmed"
            color="text-green-700"
            bg="bg-green-50 border-green-200"
          />
          <StatCard
            value={activity.total_attended}
            label="Attended"
            color="text-blue-700"
            bg="bg-blue-50 border-blue-200"
          />
          <StatCard
            value={activity.total_upcoming}
            label="Upcoming"
        ...
        <div className="flex flex-col lg:flex-row gap-5">
          <div className="flex-1 flex flex-col gap-5">
            <SectionCard title="Identity information">
              <div className="flex flex-col">
                ...
                <InfoRow
                  label="Course"
                  value={`${user.course?.name ?? "N/A"} (${user.course?.code ?? "N/A"})`}
                />
                <InfoRow
                  label="Year level"
                  value={user.year_level ? yearLabel : "N/A"}
                />
                <InfoRow
                  label="Section"
                  value={user.section ? `Section ${user.section}` : "N/A"}
                />
                <InfoRow
                  label="Account role"
                  value={
          ...
          <div className="lg:w-[340px] flex flex-col gap-5 flex-shrink-0">
            <SectionCard
              title={`Organization memberships (${memberships.length})`}
            >
              {memberships.length === 0 ? (
                <div className="flex flex-col items-center text-center py-6 gap-2">
                  ...
                  <Link
                    href="/organizations"
                    className="text-[13px] font-semibold text-green-700 hover:underline no-underline"
                  >
                ...
                <div className="flex flex-col gap-3">
                  {memberships.map((m) => (
                    <Link
                      key={m.org_id}
                      href={`/organizations/${m.org_slug ?? m.org_id}`}
                      className={`group flex items-center gap-3 p-3 rounded-xl border transition-all duration-150 no-underline ${m.is_active ? "border-gray-200 hover:border-green-300 hover:bg-green-50" : "border-gray-100 bg-gray-50 opacity-60"}`}
                    >
                      <div
                        className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 text-[12px] font-bold ${m.org_color}`}
                      >
                        {m.org_name
                          .split(" ")
                          .map((w) => w[0])
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <span
                            className={`text-[10px] font-semibold px-1.5 py-0.5 rounded border ${ORG_CATEGORY_COLORS[m.org_category]}`}
                          >
                            {m.org_category}
                          </span>
