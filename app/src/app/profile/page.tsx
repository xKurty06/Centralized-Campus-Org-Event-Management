...
/* ----------------------------------------------------------------
   ...
   Users table:
     id, school_id, email, first_name, last_name,
     dept_id → College(name, code),
     year_level, global_role, is_active

   Org_Officers table (joined):
     org_id → Organizations(name, logo_url, accreditation_status)
     position, is_active

   Registrations table (aggregated):
     count of confirmed, upcoming, attended
   ---------------------------------------------------------------- */
...
interface OrgMembership {
  ...
  org_color: string;
  position: string;        // from Org_Officers.position
  is_active: boolean;      // from Org_Officers.is_active
}
...
/* From Org_Officers JOIN Organizations */
const MOCK_MEMBERSHIPS: OrgMembership[] = [
  {
    ...
    org_status: 'Active',
    org_color: 'bg-blue-100 text-blue-700',
    position: 'Vice President',
    is_active: true,
  },
  {
    org_id: 'gdsc',
    org_name: 'Google Developer Student Club',
    org_category: 'Academic',
    org_status: 'Active',
    org_color: 'bg-red-100 text-red-700',
    position: 'Member',
    is_active: true,
  },
  {
    org_id: 'rcy',
    org_name: 'Red Cross Youth',
    org_category: 'Non-Academic',
    org_status: 'Active',
    org_color: 'bg-red-50 text-red-600',
    position: 'Member',
    is_active: false, // stepped down / inactive
  },
];

/* Aggregated from Registrations */
...
function InfoRow({
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 py-3 border-b border-gray-50 last:border-b-0">
      <span className="text-[12px] font-medium text-gray-400 flex-shrink-0 sm:w-40">{label}</span>
...
export default function ProfilePage() {
  ...
  const activeOrgs = memberships.filter((m) => m.is_active);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 w-full max-w-[1280px] mx-auto px-6 lg:px-12 py-8 flex flex-col gap-6 animate-fade-in">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          ...
          {/* Green top strip */}
          <div className="h-20 bg-green-700 relative">
            <div
          ...
          <div className="px-6 pb-6">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 -mt-10 mb-5">
              {/* Avatar */}
              <div className="w-20 h-20 rounded-2xl border-4 border-white shadow-md bg-green-100 flex items-center justify-center flex-shrink-0">
                <span className="text-[24px] font-bold text-green-700">{initials}</span>
              </div>
            ...
            <p className="text-[14px] text-gray-400 mt-0.5">
              {yearLabel} · {user.college.name} ({user.college.code})
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <StatCard value={activity.total_upcoming}   label="Upcoming events"  color="text-purple-700" bg="bg-purple-50 border-purple-200" />
        </div>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          {/* Position — green if officer, gray if member */}
...
function QuickLink({
  ...
  return (
    <Link
      href={href}
      className="group flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:border-green-200 hover:bg-green-50 transition-all no-underline"
    >
